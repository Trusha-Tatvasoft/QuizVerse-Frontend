...
describe('FeaturedQuizComponent', () => {
  ...
  it('should call playQuiz with selected quiz', () => {
    const quiz = mockQuizzes[0] as any;
    const spy = jest.spyOn(component, 'playQuiz');

    component.playQuiz(quiz);

    expect(spy).toHaveBeenCalledWith(quiz);
  });
});
